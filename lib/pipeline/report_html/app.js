(function() {
  ...
  // JS mirror of the exit-code taxonomy in lib/pipeline/error.sh (the Bash
  // single source of truth). Keep this table in sync when error.sh changes --
  // the codes are stable and few. failureBanner uses it to translate a stage's
  // exit code into a human reason instead of a bare "check the job logs".
  const EXIT_CODE_LABELS = {
    1:  'generic failure',
    2:  'invalid input',
    3:  'required tool or dependency missing',
    4:  'environment misconfiguration',
    5:  'external command or service failed',
    6:  'filesystem I/O failure',
    7:  'brik.yml configuration error',
    8:  'operation timed out',
    9:  'interrupted',
    10: 'quality or security threshold exceeded'
  };

  function failureBanner(s) {
    if (s.status !== 'failed') return '';
    const t = s.tech || {};
    const ec = (t.exit_code != null) ? t.exit_code : (s.rc != null ? s.rc : '?');
    const job = (s.runner && s.runner.job_url) || '';
    const cta = job
      ? '<a class="cta" href="' + escapeHtml(job) + '" target="_blank" rel="noopener">View job logs &rarr;</a>'
      : '';
    const codeSpan = '<span class="code">code ' + escapeHtml(String(ec)) + '</span>';
    const reasonFn = STAGE_FAILURE_REASONS[s.stage];
    const specific = reasonFn ? reasonFn(s) : null;
    let reasonInner;
    if (specific) {
      // A stage-specific builder (lint, release, ...) already maps the code
      // to a precise sentence; trust it.
      reasonInner = escapeHtml(specific);
    } else if (t.tool_error === true || t.tool_error === 'true') {
      // The scanner produced no valid report (verify.scan stamped tool_error):
      // a tool failure, not a threshold breach -- never label it "code 10".
      reasonInner = 'Scanner error: the tool produced no valid report. Check the job logs for the root cause.';
    } else {
      const label = EXIT_CODE_LABELS[ec];
      reasonInner = label
        ? escapeHtml(label) + ' (' + codeSpan + '). Check the job logs for details.'
        : 'Stage exited with ' + codeSpan + '. Check the job logs for the root cause.';
    }
    return '<div class="failure-banner">'
      + '<span class="label">Failed</span>'
      + '<span class="reason">' + reasonInner + '</span>'
      + cta
      + '</div>';
  }
  ...
  // Tile for a planned stage that never executed because a transitive
  // dependency-graph ancestor failed (or it was never reached). The
  // canonical lifecycle is stamped once at aggregation
  // (lib/pipeline/report/lifecycle.sh); we only READ s.lifecycle here and
  // surface its s.lifecycle_reason verbatim -- never reclassify. Distinct
  // from "skipped" (a planner choice) and from "running" (in flight): a
  // blocked stage must read neither as "0 findings" nor as "still in flight".
  function renderNotRunTile(s) {
    const reason = (s && s.lifecycle_reason)
      ? s.lifecycle_reason
      : 'blocked by upstream failure';
    return '<div class="not-run-banner">'
      + '<span class="label">Not run</span>'
      + '<span class="reason">' + escapeHtml(reason) + '</span>'
      + '</div>';
  }

  // A technically-failed stage that recorded no findings payload must not be
  // forwarded to a per-stage renderer that would invent a "0 findings" count
  // from the empty fragment (the node-full-cve contradiction). Surface the
  // gap honestly and point at the logs; the failure banner is rendered above.
  function renderUnavailableNote() {
    return '<div class="findings-empty results-unavailable">'
      + 'Results unavailable -- see job logs.'
      + '</div>';
  }

  // True when a business payload carries no findings/results-bearing data
  // (only status/reason metadata, or nothing at all). Used to keep a failed
  // stage from rendering deceptive zeroed counters when its fragment never
  // captured results.
  function findingsEmpty(b) {
    if (!b) return true;
    const keys = Object.keys(b).filter((k) => k !== 'status' && k !== 'reason');
    if (keys.length === 0) return true;
    // A present-but-empty findings object (no total, no items) carries no real
    // results; treat it as empty so a failed stage shows the honest note
    // rather than a "0 findings" count invented from an empty fragment.
    if (keys.length === 1 && keys[0] === 'findings') {
      const f = b.findings || {};
      const hasTotal = f.total != null;
  ...
  function renderBusiness() {
    ...
    orderedIds.forEach((sid) => {
      ...
      // The canonical per-stage lifecycle is the single source of truth,
      // stamped once at aggregation (lib/pipeline/report/lifecycle.sh). Read
      // it; never reclassify here. Fall back to the legacy tech/plan fields
      // only for pre-lifecycle aggregates (archived reports without the field).
      const lifecycle = (recorded && recorded.lifecycle) || null;
      const isSkipped = lifecycle === 'skipped'
        || (!lifecycle && (techStatus === 'skipped' || planSkipped));
      const isNotRun = lifecycle === 'not_run';
      // In-flight detection: the stage rendering the report is deliberately
      // left ABSENT from stages[] (no fragment, no stamped lifecycle), while
      // every upstream-blocked stage is synthesized as not_run and is
      // therefore present. So a planned-run stage with no recorded fragment
      // is the in-flight one. lifecycle === 'running' is honored too, though
      // stamping never emits it into stages[] today.
      const isRunning = lifecycle === 'running'
        || (!recorded && planEntry && planEntry.decision === 'run');
      ...
      const b = s.business || {};
      const isFailed = lifecycle === 'failed' || techStatus === 'failed' || s.status === 'failed';
      const isToolError = !!(s.tech && (s.tech.tool_error === true || s.tech.tool_error === 'true'));
      let inner = null;
      if (isNotRun) {
        // Planned but never executed (upstream failure / not reached): show
        // the stamped reason, not a per-stage renderer's invented zeros.
        inner = renderNotRunTile(s);
      } else if (isSkipped) {
      ...
      } else if (isFailed && (isToolError || findingsEmpty(b))) {
        // A failed stage that either produced no valid report (tool_error) or
        // carries no usable findings payload: render an honest note instead of
        // letting a per-stage renderer print a deceptive "0 findings".
        inner = renderUnavailableNote();
      } else {
      ...
      // The stage-dot colour follows the canonical lifecycle for the synthetic
      // states (not_run, running) that carry a placeholder status:'skipped' --
      // so a blocked stage's dot reads as "blocked", matching its amber
      // "Not run" banner, instead of the grey skipped dot. Real recorded
      // stages keep their technical status colour.
      const headerClass = (lifecycle === 'not_run' || lifecycle === 'running')
        ? lifecycle
        : status;
      ...
      const dryChip = sectionDryRun
        ...
        : '';
      wrap.innerHTML = '<h3 class="' + headerClass + '"><span class="stage-dot"></span><span class="stage-name">' + escapeHtml(sid) + '</span>' + dryChip + dur + '</h3>'
        + failureBanner(s)
        + inner;
