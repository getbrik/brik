(function() {
  ...
  function findingsEmpty(b) {
    if (!b) return true;
    return Object.keys(b).filter((k) => k !== 'status' && k !== 'reason').length === 0;
  }
  ...
  function renderBusiness() {
    ...
    orderedIds.forEach((sid) => {
      ...
      } else if (isFailed && findingsEmpty(b)) {
        // Failed with no findings payload: render an honest note instead of
