...
[[ -n "${_BRIK_BOOTSTRAP_LOADED:-}" ]] && return 0
_BRIK_BOOTSTRAP_LOADED=1
...
_bootstrap._detect_system_pkg_manager() {
    local mgr
...
# Usage: _bootstrap._self_host_binary <name> <url>
_bootstrap._self_host_binary() {
    local name="$1"
    local url="$2"
...
# Usage: _bootstrap._sys_pkg_install <manager> <logical_name> [version]
_bootstrap._sys_pkg_install() {
    local mgr="$1"
    local name="$2"
    local version="${3:-}"

    local cmd
    cmd="$(_bootstrap._tool_command "$name")"
    ...
    case "$mgr" in
        apk)     _bootstrap._install_via_apk "$name" "$version" ;;
        apt-get) _bootstrap._install_via_apt "$name" "$version" ;;
        yum)     _bootstrap._install_via_yum "$name" "$version" ;;
        dnf)     _bootstrap._install_via_dnf "$name" "$version" ;;
        *)
...
bootstrap.install_yq() {
    ...
    local url
    url="$(_bootstrap._yq_url)"

    if _bootstrap._is_virtualized; then
        local mgr
        mgr="$(_bootstrap._detect_system_pkg_manager)"
        ...
        _bootstrap._self_host_binary "yq" "$url"
    fi
...
bootstrap.install_prerequisites() {
    if _bootstrap._is_brik_runner; then
    ...
    if ! _bootstrap._is_virtualized; then
        # best-effort: PATH setup is non-critical
        _bootstrap._ensure_brik_bin || true
    fi
    ...
    if _bootstrap._is_virtualized; then
        ...
        for tool in jq git bash; do
            local cmd
            cmd="$(_bootstrap._tool_command "$tool")"
            if command -v "$cmd" >/dev/null 2>&1; then
                log.debug "$tool already installed"
                continue
            fi
            if [[ -n "$mgr" ]]; then
                _bootstrap._sys_pkg_install "$mgr" "$tool" || {
        ...
        if ! command -v jq >/dev/null 2>&1; then
            local jq_url
            jq_url="$(_bootstrap._jq_url)"
            _bootstrap._self_host_binary "jq" "$jq_url" || {
...
bootstrap.install_stack() {
    ...
    local cmd
    cmd="$(_bootstrap._tool_command "$stack")"
    ...
    if _bootstrap._is_virtualized; then
        ...
        if [[ -z "$mgr" ]]; then
            ...
            bootstrap.check_stack "$stack"
            return $?
        fi
        _bootstrap._sys_pkg_install "$mgr" "$stack" "$version" || {
            log.error "failed to install stack '$stack' via $mgr"
            return "$BRIK_EXIT_EXTERNAL_FAIL"
        }
        # Post-install hooks (CI only - ephemeral environment)
        case "$stack" in
            python) _bootstrap._python_post_install ;;
        esac
    else
        # Local: mise if available, otherwise check-and-fail
        if command -v mise >/dev/null 2>&1; then
            _bootstrap._install_via_mise "$stack" "${version:-latest}" || {
...
_bootstrap._python_post_install() {
    if ! _bootstrap._is_virtualized; then
        return 0
    fi
...
# Usage: bootstrap.prepare_env [stack]
bootstrap.prepare_env() {
    ...
    if [[ -n "$stack" ]]; then
        local stack_rc=0
        bootstrap.install_stack "$stack" || stack_rc=$?
