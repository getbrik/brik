...
[[ -n "${_BRIK_VERIFY_TYPE_CHECK_LOADED:-}" ]] && return 0
_BRIK_VERIFY_TYPE_CHECK_LOADED=1

# Run type checking on a workspace.
# Usage: verify.type_check.run <workspace>
verify.type_check.run() {
    local workspace="$1"
    shift
