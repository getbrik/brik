#!/usr/bin/env bash
# @module stages/verify/scan
# @description Security facade. Dispatches scans to dedicated sub-modules.
# Pattern: same as verify.run --checks (parameterized dispatch).

# Guard against double-sourcing
[[ -n "${_BRIK_STAGES_VERIFY_SCAN_LOADED:-}" ]] && return 0
_BRIK_STAGES_VERIFY_SCAN_LOADED=1

# Run security scans on a workspace.
# Usage: verify.scan.run <workspace> [--scans <deps,secret,sast,license,iac,container>]
...
verify.scan.run() {
    ...
    for scan in "${_scan_list[@]}"; do
        ...
        scan_fn="verify.scan.${module_name}.run"
        if ! declare -f "$scan_fn" >/dev/null 2>&1; then
            brik.use "verify.scan.${module_name}" || true
        fi

        if ! declare -f "$scan_fn" >/dev/null 2>&1; then
            log.warn "verify.scan.${module_name} module not available - skipping $scan scan"
            continue
        fi
