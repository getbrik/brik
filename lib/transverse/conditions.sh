...
# Loaded via: brik.use conditions
#
...
# Usage: conditions.eval <expression>
...
#
# Examples:
#   conditions.eval "branch == 'main'"
#   conditions.eval "tag =~ 'v*'"
#   conditions.eval "manual"
#   conditions.eval "branch == 'main' AND tag =~ 'v*'"
#   conditions.eval "branch == 'main' OR branch == 'develop'"
#   conditions.eval "branch == 'main' AND tag =~ 'v*' OR branch == 'develop'"
conditions.eval() {
    local expression="$1"
    ...
    if [[ "$expression" == *" OR "* ]]; then
        ...
        conditions.eval "$or_left" && return 0
        # Evaluate right part (which may contain AND or OR)
        conditions.eval "$or_right"
        return $?
    fi
    ...
    if [[ "$expression" == *" AND "* ]]; then
        ...
        conditions.eval "$and_left" || return "$BRIK_EXIT_FAILURE"
        conditions.eval "$and_right"
        return $?
    fi
...
# Usage: conditions.eval_deploy_env <env_name>
...
conditions.eval_deploy_env() {
    ...
    if ! declare -f config.get >/dev/null 2>&1; then
        brik.use config || {
            log.error "config module is required for conditions.eval_deploy_env"
            return "$BRIK_EXIT_FAILURE"
        }
    fi
