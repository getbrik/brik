...
that plan -- GitLab via a dynamic child pipeline. Adds the `promote`
stage, the `brik plan` and `brik extension` commands, user-supplied
extensions, and a GitLab `workflow:` filter. The legacy GitLab template
is deprecated in favour of the planner-driven one.
...
- **Planner and `plan.json`** -- a new planning layer (`lib/planning/`)
  ...
  `balanced` mode. New CLI: `brik plan` (with `--explain`,
  `--validate-only`, `--format gitlab-child`) and `brik run pipeline
  --plan` / `--auto-select`. Plan output is reproducible (byte-identical
  on an unchanged HEAD).
- **Plan-driven adapters** -- the local, Jenkins and GitLab adapters all
  execute from `plan.json`. GitLab gains a dynamic child pipeline
  generated by a `brik-plan` parent job (`dynamic-pipeline.yml`); Jenkins
  reads the plan and gates each stage, treating a planner failure as
  non-fatal (logs the rejection and falls back to the legacy fixed flow).
...
- **GitLab `workflow:` filter** -- both `pipeline.yml` and
  `dynamic-pipeline.yml` declare a top-level `workflow:` block that
  filters parasite pipelines upstream (anti-duplicate push+MR via
  `$CI_OPEN_MERGE_REQUESTS`, allow-list of legitimate
  `$CI_PIPELINE_SOURCE` values including `parent_pipeline`, explicit
  `schedule` + `web`). Documented in `docs/platforms/gitlab.md` with a
  project-side anti-patterns checklist.
...
- **Legacy GitLab template** -- `shared-libs/gitlab/templates/pipeline.yml`
  is deprecated in favour of the planner-driven `dynamic-pipeline.yml`. It
  is kept for two minor versions and requires no `brik.yml` change to
  migrate.
