Describe "publish/maven.sh"
  ...
  Describe "pkg.maven.publish"
    It "returns 2 for unknown option"
      When call pkg.maven.publish --badopt foo
    ...
    Describe "with mock mvn"
      ...
      It "runs mvn deploy"
        invoke_deploy() {
          pkg.maven.publish 2>/dev/null || return 1
      ...
      It "passes repository option"
        invoke_repo() {
          pkg.maven.publish --repository "https://maven.example.com" 2>/dev/null || return 1
      ...
      It "uses dry-run mode"
        When call pkg.maven.publish --dry-run
        The status should be success
      ...
      It "writes temporary settings.xml with credentials"
        invoke_creds() {
          ...
          pkg.maven.publish --username-var "MY_MVN_USER" --password-var "MY_MVN_PASS" 2>/dev/null || return 1
      ...
      It "returns 7 when username_var references unset variable"
        When call pkg.maven.publish --username-var "NONEXISTENT_VAR_12345"
      ...
      It "returns 7 when password_var references unset variable"
        invoke_bad_pass() {
          export MY_MVN_USER="admin"
          pkg.maven.publish --username-var "MY_MVN_USER" --password-var "NONEXISTENT_VAR_12345"
        }
