Describe "publish/nuget.sh"
  ...
  Describe "pkg.nuget.publish"
    It "returns 2 for unknown option"
      When call pkg.nuget.publish --badopt foo
    ...
    Describe "with mock dotnet"
      ...
      It "runs dotnet nuget push"
        invoke_push() {
          pkg.nuget.publish 2>/dev/null || return 1
      ...
      It "passes source option"
        invoke_source() {
          pkg.nuget.publish --source "https://api.nuget.org/v3/index.json" 2>/dev/null || return 1
      ...
      It "passes api key"
        invoke_key() {
          ...
          pkg.nuget.publish --token-var "MY_NUGET_KEY" 2>/dev/null || return 1
      ...
      It "uses dry-run mode"
        When call pkg.nuget.publish --dry-run
        The status should be success
...
FAILEOF
    ...
    Describe "with HTTP source (NuGet.Config)"
      ...
      It "creates NuGet.Config for HTTP source"
        invoke_http() {
          pkg.nuget.publish --source "http://nexus:8081/repository/nuget/" 2>/dev/null || return 1
      ...
      It "uses basic auth when api key contains colon"
        invoke_basic_auth() {
          ...
          pkg.nuget.publish --source "http://nexus:8081/repository/nuget/" --token-var "MY_HTTP_KEY" 2>/dev/null || return 1
      ...
      It "dry-run with HTTP source"
        When call pkg.nuget.publish --source "http://nexus:8081/repository/nuget/" --dry-run
        The status should be success
        The stderr should include "[dry-run]"
      End
    End
