Describe "publish/docker.sh"
  ...
  Describe "pkg.docker.publish"
    It "returns 2 for unknown option"
      When call pkg.docker.publish --badopt foo
      The status should equal 2
      The stderr should include "unknown option"
    End

    It "returns 2 when no image specified"
      When call pkg.docker.publish
      The status should equal 2
    ...
    Describe "require_tool docker failure"
      ...
      It "returns 3 when docker is not on PATH"
        When call pkg.docker.publish --image myapp
    ...
    Describe "with mock docker"
      ...
      It "pushes image with specified tag"
        invoke_push() {
          pkg.docker.publish --image "myapp" --tags "v1.0.0" 2>/dev/null || return 1
      ...
      It "defaults tags to BRIK_APP_VERSION"
        invoke_version_tag() {
          ...
          pkg.docker.publish --image "myapp" 2>/dev/null || return 1
      ...
      It "pushes multiple tags"
        invoke_multi_tag() {
          pkg.docker.publish --image "myapp" --tags "v1.0.0,latest" 2>/dev/null || return 1
      ...
      It "returns 7 when username_var references unset variable"
        When call pkg.docker.publish --image "myapp" --username-var "NONEXISTENT_USER" --password-var "NONEXISTENT_PASS"
      ...
      It "reads image from BRIK_PUBLISH_DOCKER_IMAGE"
        invoke_env_image() {
          ...
          pkg.docker.publish --tags "v1.0.0" 2>/dev/null || return 1
      ...
      It "uses --dry-run flag"
        When call pkg.docker.publish --image "myapp" --tags "v1.0.0" --dry-run
        The status should be success
      ...
      It "does not call docker push in dry-run"
        invoke_no_push() {
          pkg.docker.publish --image "myapp" --tags "v1.0.0" --dry-run 2>/dev/null || return 1
      ...
      It "reports success"
        When call pkg.docker.publish --image "myapp" --tags "v1.0.0"
        The status should be success
