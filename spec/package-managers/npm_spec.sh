Describe "publish/npm.sh"
  ...
  Describe "pkg.npm.publish"
    It "returns 2 for unknown option"
      When call pkg.npm.publish --badopt foo
    ...
    Describe "with mock npm"
      ...
      It "passes --registry option"
        invoke_registry() {
          pkg.npm.publish --registry "https://npm.example.com" 2>/dev/null || return 1
      ...
      It "passes --tag option"
        invoke_tag() {
          pkg.npm.publish --tag "beta" 2>/dev/null || return 1
      ...
      It "defaults tag to latest"
        invoke_default_tag() {
          pkg.npm.publish 2>/dev/null || return 1
      ...
      It "passes --access option"
        invoke_access() {
          pkg.npm.publish --access "public" 2>/dev/null || return 1
      ...
      It "sets NPM_TOKEN from token_var and cleans up after publish"
        invoke_token() {
          ...
          pkg.npm.publish --token-var "MY_NPM_TOKEN" 2>/dev/null || return 1
      ...
      It "returns 7 when token_var references unset variable"
        When call pkg.npm.publish --token-var "NONEXISTENT_VAR_12345"
      ...
      It "uses --dry-run flag"
        invoke_dryrun() {
          pkg.npm.publish --dry-run 2>/dev/null || return 1
