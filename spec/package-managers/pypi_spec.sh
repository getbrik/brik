Describe "publish/pypi.sh"
  ...
  Describe "pkg.pypi.publish"
    It "returns 2 for unknown option"
      When call pkg.pypi.publish --badopt foo
    ...
    Describe "with mock twine"
      ...
      It "runs twine upload"
        invoke_twine() {
          pkg.pypi.publish 2>/dev/null || return 1
      ...
      It "passes repository URL"
        invoke_repo() {
          pkg.pypi.publish --repository "https://test.pypi.org/legacy/" 2>/dev/null || return 1
      ...
      It "sets TWINE_USERNAME and TWINE_PASSWORD env vars (not CLI args)"
        invoke_token() {
          ...
          pkg.pypi.publish --token-var "MY_PYPI_TOKEN" 2>/dev/null || return 1
      ...
      It "uses dry-run mode"
        When call pkg.pypi.publish --dry-run
        The status should be success
    ...
    Describe "twine with basic auth (user:password)"
      ...
      It "sets TWINE_USERNAME and TWINE_PASSWORD from user:password format"
        invoke_basic() {
          ...
          pkg.pypi.publish --token-var "MY_BASIC_TOKEN" 2>/dev/null || return 1
