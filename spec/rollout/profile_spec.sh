Describe "deploy/profile.sh"
  ...
  Describe "rollout.profile.resolve"
    ...
    It "profile path exists on disk for trunk-based"
      check_exists() {
        local path
        path="$(rollout.profile.resolve "trunk-based")" || return 1
    ...
    It "profile path exists on disk for git-flow"
      check_exists() {
        local path
        path="$(rollout.profile.resolve "git-flow")" || return 1
        [[ -f "$path" ]] && echo "exists" || echo "missing"
      }
      When call check_exists
      The output should equal "exists"
    ...
    It "profile path exists on disk for github-flow"
      check_exists() {
        local path
        path="$(rollout.profile.resolve "github-flow")" || return 1
        [[ -f "$path" ]] && echo "exists" || echo "missing"
      }
      When call check_exists
      The output should equal "exists"
  ...
  # =========================================================================
  Describe "rollout.profile.merge"
    setup_yq() {
...
YAML
      ...
      It "returns a temp file path"
        check_merge() {
          local out
          out="$(rollout.profile.merge "trunk-based" "$TEMP_CONFIG")" || return 1
      ...
      It "git-flow merge contains dev environment"
        check_dev() {
          local merged
          merged="$(rollout.profile.merge "git-flow" "$TEMP_CONFIG")" || return 1
      ...
      It "github-flow merge contains preview environment"
        check_preview() {
          local merged
          merged="$(rollout.profile.merge "github-flow" "$TEMP_CONFIG")" || return 1
...
YAML
    ...
    Describe "error handling"
      It "returns 2 for empty workflow"
        When call rollout.profile.merge "" "/some/path.yml"
        The status should equal 2
        The stderr should include "workflow is required"
      End

      It "returns 6 for non-existent brik.yml"
        When call rollout.profile.merge "trunk-based" "/nonexistent/brik.yml"
      ...
      It "returns 2 for unknown workflow"
        check_unknown() {
          ...
          tmp="$(mktemp)"
          printf 'version: 1\n' > "$tmp"
          rollout.profile.merge "bad-workflow" "$tmp"
          local rc=$?
          rm -f "$tmp"
          return $rc
        }
  ...
  Describe "double-sourcing guard"
    It "is callable after double include"
      double_include() {
        ...
        . "$BRIK_ROLLOUT_LIB/profile.sh"
        declare -f rollout.profile.resolve >/dev/null && echo "ok" || echo "missing"
      }
