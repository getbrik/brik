Describe "deploy/health.sh"
  ...
  # =========================================================================
  Describe "rollout.health.check"
    It "returns 2 when --url is missing"
      When call rollout.health.check
    ...
    Describe "require_tool curl failure"
      ...
      It "returns 3 when curl is not on PATH"
        When call rollout.health.check --url "http://example.com/health"
    ...
    Describe "with mock curl returning 200"
      ...
      It "returns 0 when status matches expected 200"
        invoke_check_200() { rollout.health.check --url "http://example.com/health" 2>/dev/null; }
        When call invoke_check_200
        The status should be success
      End

      It "returns 0 with explicit --expected-status 200"
        invoke_check_explicit() { rollout.health.check --url "http://example.com/health" --expected-status 200 2>/dev/null; }
    ...
    Describe "with mock curl returning 503"
      ...
      It "returns BRIK_EXIT_CHECK_FAILED when status does not match expected"
        invoke_check_503() { rollout.health.check --url "http://example.com/health" 2>/dev/null; }
    ...
    Describe "with mock curl returning 201"
      ...
      It "returns 0 when status matches custom --expected-status 201"
        invoke_check_201() { rollout.health.check --url "http://example.com/health" --expected-status 201 2>/dev/null; }
        When call invoke_check_201
        The status should be success
      End

      It "returns BRIK_EXIT_CHECK_FAILED when status does not match custom --expected-status 200"
        invoke_check_mismatch() { rollout.health.check --url "http://example.com/health" --expected-status 200 2>/dev/null; }
  ...
  # rollout.health.wait
  # =========================================================================
  Describe "rollout.health.wait"
    It "returns 2 when --url is missing"
      When call rollout.health.wait
      The status should equal 2
      The stderr should include "url is required"
    End
    ...
    Describe "with mock curl always returning 200 (immediate success)"
      ...
      It "returns 0 when healthy before timeout"
        invoke_wait_200() { rollout.health.wait --url "http://example.com/health" --timeout 10 --interval 1 2>/dev/null; }
        When call invoke_wait_200
        The status should be success
      End

      It "succeeds and logs health check"
        When call rollout.health.wait --url "http://example.com/health" --timeout 10 --interval 1
        The status should be success
    ...
    Describe "with mock curl always returning 503 (timeout scenario)"
      ...
      It "returns BRIK_EXIT_TIMEOUT when timeout is reached without healthy response"
        ...
        When call rollout.health.wait --url "http://example.com/health" --timeout 2 --interval 1
    ...
    Describe "dry-run mode"
      ...
      It "logs dry-run message without polling"
        When call rollout.health.wait --url "http://example.com/health"
        The status should be success
  ...
  # =========================================================================
  Describe "rollout.health.k8s_wait"
    It "returns 2 when --namespace is missing"
      When call rollout.health.k8s_wait --deployment my-app
      The status should equal 2
      The stderr should include "namespace is required"
    End

    It "returns 2 when --deployment is missing"
      When call rollout.health.k8s_wait --namespace production
      The status should equal 2
    ...
    Describe "require_tool kubectl failure"
      ...
      It "returns 3 when kubectl is not on PATH"
        When call rollout.health.k8s_wait --namespace production --deployment my-app
        The status should equal 3
        The stderr should include "required tool not found"
      End
    End
    ...
    Describe "with mock kubectl"
      ...
      It "calls kubectl rollout status"
        invoke_k8s_wait() {
          rollout.health.k8s_wait --namespace production --deployment my-app 2>/dev/null || return 1
  ...
  # =========================================================================
  # rollout.health.check - additional coverage
  # =========================================================================
  Describe "rollout.health.check - input validation"
    It "returns 2 for unknown option"
      When call rollout.health.check --badopt foo
      The status should equal 2
      The stderr should include "unknown option"
    End

    It "returns 2 for invalid URL scheme (ftp)"
      When call rollout.health.check --url "ftp://example.com/health"
      The status should equal 2
      The stderr should include "http or https"
    End

    It "returns 2 for invalid expected-status (non-3-digit)"
      When call rollout.health.check --url "http://example.com/health" --expected-status "abc"
      The status should equal 2
      The stderr should include "3-digit"
    End

    It "returns 2 for expected-status with too few digits"
      When call rollout.health.check --url "http://example.com/health" --expected-status "20"
      The status should equal 2
      The stderr should include "3-digit"
    End
    ...
    Describe "dry-run mode"
      It "logs dry-run message and returns 0"
        When call rollout.health.check --url "http://example.com/health" --dry-run
        The status should be success
        The stderr should include "dry-run"
      End
    End
  ...
  # =========================================================================
  # rollout.health.wait - input validation
  # =========================================================================
  Describe "rollout.health.wait - input validation"
    It "returns 2 for unknown option"
      When call rollout.health.wait --badopt foo
      The status should equal 2
      The stderr should include "unknown option"
    End

    It "returns 2 for invalid URL scheme"
      When call rollout.health.wait --url "ftp://example.com/health"
      The status should equal 2
      The stderr should include "http or https"
    End

    It "returns 2 for non-integer timeout"
      When call rollout.health.wait --url "http://example.com/health" --timeout "abc"
      The status should equal 2
      The stderr should include "timeout must be a positive integer"
    End

    It "returns 2 for non-integer interval"
      When call rollout.health.wait --url "http://example.com/health" --interval "abc"
      The status should equal 2
      The stderr should include "interval must be a positive integer"
    End

    It "returns 2 for interval of 0"
      When call rollout.health.wait --url "http://example.com/health" --interval 0
      The status should equal 2
      The stderr should include "interval must be a positive integer"
    End

    It "returns 2 for invalid expected-status in wait"
      When call rollout.health.wait --url "http://example.com/health" --expected-status "abc"
      The status should equal 2
      The stderr should include "3-digit"
    End
  End

  Describe "rollout.health.wait - require_tool failure"
    setup_no_curl_wait() {
      mock.setup
      mock.isolate
    }
  ...
  Describe "rollout.health.wait - --dry-run flag"
    setup_dryrun_flag() {
      mock.setup
      mock.create_output "curl" "200"
      mock.activate
    ...
    It "logs dry-run message with --dry-run flag (not env)"
      When call rollout.health.wait --url "http://example.com/health" --dry-run
      The status should be success
      The stderr should include "dry-run"
    End
  End
  ...
  # =========================================================================
  # rollout.health.k8s_wait - input validation
  # =========================================================================
  Describe "rollout.health.k8s_wait - input validation"
    It "returns 2 for unknown option"
      When call rollout.health.k8s_wait --badopt foo
      The status should equal 2
      The stderr should include "unknown option"
    End

    It "returns 2 for non-integer timeout"
      When call rollout.health.k8s_wait --namespace production --deployment my-app --timeout "abc"
      The status should equal 2
      The stderr should include "timeout must be a positive integer"
    End
  End

  Describe "rollout.health.k8s_wait - custom timeout"
    ...
    It "passes custom timeout to kubectl"
      invoke_custom_timeout() {
        rollout.health.k8s_wait --namespace production --deployment my-app --timeout 60 2>/dev/null || return 1
        grep -q "60s" "$MOCK_LOG"
      }
  ...
  Describe "rollout.health.k8s_wait - kubectl failure"
    setup_kubectl_fail() {
      mock.setup
  ...
  Describe "rollout.health.k8s_wait - passthrough options"
    setup_kubectl_pass() {
      mock.setup
      TEST_WS="$(mktemp -d)"
      MOCK_LOG="${TEST_WS}/mock_kubectl.log"
    ...
    It "ignores --target and --env passthrough options"
      When call rollout.health.k8s_wait --namespace production --deployment my-app --target k8s --env staging
      The status should be success
      The stderr should include "rollout"
    End
  End
  ...
  Describe "double-sourcing guard"
    It "is callable after double include"
      double_include() {
        ...
        . "$BRIK_ROLLOUT_LIB/health.sh"
        declare -f rollout.health.check >/dev/null && echo "ok" || echo "missing"
      }
