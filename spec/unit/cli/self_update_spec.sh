...
Describe "brik self-update"
  ...
  Describe "unknown install method"
    setup_unknown() {
      ...
      cp -R "${BRIK_HOME}/lib" "${FAKE_HOME_UP}/lib" 2>/dev/null || true
  ...
  Describe "git dirty working tree"
    setup_dirty_git() {
      ...
      cp -R "${BRIK_HOME}/lib" "${FAKE_GIT_HOME}/lib" 2>/dev/null || true
  ...
  Describe "git update stable with no tags"
    setup_no_tags() {
      ...
      cp -R "${BRIK_HOME}/lib" "${FAKE_NT_HOME}/lib" 2>/dev/null || true
