Describe "deploy/compose.sh"
  ...
  Describe "deploy.compose.run"
    ...
    Describe "local deploy with mock docker"
      ...
      It "uses default docker-compose.yml when --file not provided"
        invoke_default_file() {
          cd "$TEST_WS" || return 1
      ...
      It "uses custom compose file when --file provided"
        invoke_custom_file() {
          deploy.compose.run --namespace myapp \
            --file "${TEST_WS}/docker-compose.yml" 2>/dev/null || return 1
          grep -q "docker-compose.yml" "$MOCK_LOG"
        }
    ...
    Describe "BRIK_DRY_RUN env var"
      ...
      It "respects BRIK_DRY_RUN env var"
        invoke_env_dryrun() {
          local log="${TEST_WS}/mock_docker.log"
          deploy.compose.run --namespace myapp \
            --file "${TEST_WS}/docker-compose.yml" 2>/dev/null
          [[ ! -f "$log" ]] || ! grep -q "up" "$log"
        }
    ...
    Describe "double-sourcing guard"
      It "is callable after double include"
        double_include() {
          ...
          . "$BRIK_DEPLOYMENTS_LIB/compose.sh"
