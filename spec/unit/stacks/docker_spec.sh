Describe "build/docker.sh"
  ...
  Include "$BRIK_STACKS_LIB/docker.sh"
  Include "$BRIK_HOME/spec/support/mock_helper.sh"

  Describe "stacks.docker.build"
    It "returns 6 for nonexistent workspace"
      When call stacks.docker.build "/nonexistent/workspace"
      The status should equal 6
      The stderr should include "required directory not found"
    End

    It "returns 2 for unknown option"
      When call stacks.docker.build "$WORKSPACES/docker-simple" --badopt foo
      The status should equal 2
      The stderr should include "unknown option"
    End

    It "returns 6 when Dockerfile is missing"
      When call stacks.docker.build "$WORKSPACES/unknown"
      The status should equal 6
    ...
    Describe "require_tool docker failure"
      ...
      It "returns 3 when docker is not on PATH"
        When call stacks.docker.build "$TEST_WS"
    ...
    Describe "with mock docker"
      ...
      It "runs docker build with default tag project:latest"
        invoke_docker_default() {
          stacks.docker.build "$TEST_WS" 2>/dev/null || return 1
      ...
      It "uses custom tag when --tag specified"
        invoke_custom_tag() {
          stacks.docker.build "$TEST_WS" --tag "myapp:1.0" 2>/dev/null || return 1
      ...
      It "uses custom Dockerfile when --file specified"
        invoke_custom_file() {
          ...
          printf 'FROM alpine:3.19\n' > "$custom"
          stacks.docker.build "$TEST_WS" --file "$custom" 2>/dev/null || return 1
      ...
      It "uses custom context when --context specified"
        invoke_custom_context() {
          ...
          mkdir -p "$ctx"
          stacks.docker.build "$TEST_WS" --context "$ctx" 2>/dev/null || return 1
          grep -q "${ctx}$" "$MOCK_LOG"
        }
      ...
      It "passes build args"
        invoke_build_args() {
          stacks.docker.build "$TEST_WS" --build-arg "VERSION=1.0" 2>/dev/null || return 1
      ...
      It "accumulates multiple build args"
        invoke_multi_args() {
          stacks.docker.build "$TEST_WS" --build-arg "A=1" --build-arg "B=2" 2>/dev/null || return 1
...
exit 1'
      ...
      It "does not execute docker in dry-run mode"
        When call stacks.docker.build "$TEST_WS" --dry-run
        The status should be success
