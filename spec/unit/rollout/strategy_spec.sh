Describe "deploy/strategy.sh"
  ...
  Describe "rollout.strategy.run"
    ...
    It "returns 2 when rollback-fn is not a declared function"
      ...
      When call rollout.strategy.run --type rolling --deploy-fn "my_deploy_fn" --rollback-fn "nonexistent_rollback"
      The status should equal 2
      The stderr should include "not a declared function"
    End

    It "returns 2 for unknown option"
      When call rollout.strategy.run --badopt foo
      The status should equal 2
  ...
  # =========================================================================
  # rollout.strategy.run - rolling strategy
  # =========================================================================
  Describe "rollout.strategy.run - rolling"
    Describe "successful deploy"
      ...
      It "calls deploy-fn and succeeds"
        When call rollout.strategy.run --type rolling --deploy-fn "test_deploy_ok --app my-app"
        The status should be success
        The stderr should include "rolling deployment completed"
      End

      It "passes args from deploy-fn string"
        When call rollout.strategy.run --type rolling --deploy-fn "test_deploy_ok --app my-app --flag"
        The status should be success
    ...
    Describe "deploy failure without rollback"
      ...
      It "returns 1 when deploy fails and no rollback-fn"
        When call rollout.strategy.run --type rolling --deploy-fn "test_deploy_fail"
    ...
    Describe "deploy failure with successful rollback"
      ...
      It "returns 5 when deploy fails but rollback succeeds"
        When call rollout.strategy.run --type rolling --deploy-fn "test_deploy_fail2" --rollback-fn "test_rollback_ok"
    ...
    Describe "deploy failure with failed rollback"
      ...
      It "returns 1 when both deploy and rollback fail"
        When call rollout.strategy.run --type rolling --deploy-fn "test_deploy_fail3" --rollback-fn "test_rollback_fail"
        The status should equal 1
        The stderr should include "deploy failed"
    ...
    Describe "dry-run mode"
      ...
      It "logs dry-run message"
        When call rollout.strategy.run --type rolling --deploy-fn "test_deploy_dryrun --app my-app" --dry-run
        The status should be success
  ...
  # =========================================================================
  # rollout.strategy.run - blue-green strategy
  # =========================================================================
  Describe "rollout.strategy.run - blue-green"
    test_bg_deploy() { return 0; }

    It "calls deploy-fn and succeeds"
      When call rollout.strategy.run --type blue-green --deploy-fn "test_bg_deploy"
      The status should be success
  ...
  # =========================================================================
  # rollout.strategy.run - canary strategy
  # =========================================================================
  Describe "rollout.strategy.run - canary"
    test_canary_deploy() { return 0; }

    It "calls deploy-fn and succeeds"
      When call rollout.strategy.run --type canary --deploy-fn "test_canary_deploy"
      The status should be success
  ...
  # =========================================================================
  # rollout.strategy.run - passthrough options
  # =========================================================================
  Describe "rollout.strategy.run - passthrough"
    test_pass_deploy() { return 0; }

    It "ignores --target and --env"
      When call rollout.strategy.run --type rolling --deploy-fn "test_pass_deploy" --target k8s --env staging
      The status should be success
      The stderr should include "rolling deployment completed"
    End
  End
  ...
  # =========================================================================
  # rollout.strategy.run - BRIK_DRY_RUN env var
  # =========================================================================
  Describe "rollout.strategy.run - BRIK_DRY_RUN env"
    ...
    It "respects BRIK_DRY_RUN env var"
      When call rollout.strategy.run --type rolling --deploy-fn "test_env_deploy"
      The status should be success
      The stderr should include "dry-run"
    End
  End
  ...
  # =========================================================================
  Describe "_rollout.strategy._rolling_kubectl"
    It "returns 2 when --deployment is missing"
      When call _rollout.strategy._rolling_kubectl
      The status should equal 2
      The stderr should include "deployment name is required"
    End

    It "returns 2 for non-integer timeout"
      When call _rollout.strategy._rolling_kubectl --deployment my-app --timeout "abc"
      The status should equal 2
    ...
    Describe "require_tool kubectl failure"
      ...
      It "returns 3 when kubectl is not on PATH"
        When call _rollout.strategy._rolling_kubectl --deployment my-app
    ...
    Describe "with mock kubectl"
      ...
      It "calls kubectl rollout status"
        invoke_rolling() {
          _rollout.strategy._rolling_kubectl --deployment my-app 2>/dev/null || return 1
      ...
      It "respects --timeout option"
        invoke_timeout() {
          _rollout.strategy._rolling_kubectl --deployment my-app --timeout 60 2>/dev/null || return 1
          grep -q "60s" "$MOCK_LOG"
        }
    ...
    Describe "passthrough options"
      ...
      It "ignores --target and --env passthrough options"
        When call _rollout.strategy._rolling_kubectl --deployment my-app --target k8s --env staging
        The status should be success
        The stderr should include "rolling"
      End
    End
  ...
  # =========================================================================
  Describe "_rollout.strategy._blue_green_kubectl"
    It "returns 2 when --service is missing"
      When call _rollout.strategy._blue_green_kubectl --namespace production
      The status should equal 2
      The stderr should include "service is required"
    End

    It "returns 2 when --target-selector is missing"
      When call _rollout.strategy._blue_green_kubectl --service my-svc
      The status should equal 2
    ...
    Describe "require_tool kubectl failure"
      ...
      It "returns 3 when kubectl is not on PATH"
        When call _rollout.strategy._blue_green_kubectl --service my-svc --target-selector version=green
        The status should equal 3
        The stderr should include "required tool not found"
      End
    End
    ...
    Describe "with mock kubectl"
      ...
      It "patches service selector for blue-green switch"
        invoke_patch() {
          _rollout.strategy._blue_green_kubectl --service my-svc --target-selector version=green 2>/dev/null || return 1
      ...
      It "passes --namespace to kubectl"
        invoke_namespace() {
          _rollout.strategy._blue_green_kubectl --service my-svc --target-selector version=green \
    ...
    Describe "selector validation"
      ...
      It "returns 2 for invalid selector key"
        When call _rollout.strategy._blue_green_kubectl --service my-svc --target-selector "bad key=green"
        The status should equal 2
        The stderr should include "invalid selector key"
      End

      It "returns 2 for invalid selector value"
        When call _rollout.strategy._blue_green_kubectl --service my-svc --target-selector "version=bad value!"
        The status should equal 2
    ...
    Describe "passthrough options"
      ...
      It "ignores --target and --env passthrough options"
        When call _rollout.strategy._blue_green_kubectl --service my-svc --target-selector version=green --target k8s --env staging
        The status should be success
        The stderr should include "blue-green"
      End
    End
  ...
  # =========================================================================
  Describe "_rollout.strategy._canary_kubectl"
    It "returns 2 when --service is missing"
      When call _rollout.strategy._canary_kubectl --namespace production
      The status should equal 2
      The stderr should include "service is required"
    End

    It "returns 2 when --deployment is missing"
      When call _rollout.strategy._canary_kubectl --service my-svc
      The status should equal 2
      The stderr should include "deployment is required"
    End

    It "returns 2 for non-integer replicas"
      When call _rollout.strategy._canary_kubectl --service my-svc --deployment my-canary --replicas "abc"
      The status should equal 2
      The stderr should include "replicas must be a positive integer"
    End

    It "returns 2 for replicas of 0"
      When call _rollout.strategy._canary_kubectl --service my-svc --deployment my-canary --replicas 0
      The status should equal 2
      The stderr should include "replicas must be a positive integer"
    End
    ...
    Describe "require_tool kubectl failure"
      ...
      It "returns 3 when kubectl is not on PATH"
        When call _rollout.strategy._canary_kubectl --service my-svc --deployment my-canary
        The status should equal 3
        The stderr should include "required tool not found"
      End
    End
    ...
    Describe "with mock kubectl"
      ...
      It "uses default replicas of 1"
        invoke_default_weight() {
          _rollout.strategy._canary_kubectl --service my-svc --deployment my-canary 2>/dev/null || return 1
          grep -q "1" "$MOCK_LOG"
        }
      ...
      It "respects --replicas option"
        invoke_weight() {
          _rollout.strategy._canary_kubectl --service my-svc --deployment my-canary --replicas 3 2>/dev/null || return 1
          grep -q "3" "$MOCK_LOG"
        }
      ...
      It "passes --namespace to kubectl"
        invoke_ns() {
          _rollout.strategy._canary_kubectl --service my-svc --deployment my-canary --namespace production 2>/dev/null || return 1
    ...
    Describe "passthrough options"
      ...
      It "ignores --target and --env passthrough options"
        When call _rollout.strategy._canary_kubectl --service my-svc --deployment my-canary --target k8s --env staging
        The status should be success
        The stderr should include "canary"
      End
    End
  ...
  Describe "double-sourcing guard"
    It "is callable after double include"
      double_include() {
        ...
        . "$BRIK_ROLLOUT_LIB/strategy.sh"
        declare -f rollout.strategy.run >/dev/null && echo "ok" || echo "missing"
      }
