Describe "security/license.sh"
  ...
  Describe "verify.scan.license.run"
    It "returns 6 for nonexistent workspace"
      When call verify.scan.license.run "/nonexistent/workspace"
    ...
    Describe "license_finder available"
      ...
      It "runs license_finder action_items"
        invoke_lf() {
          verify.scan.license.run "$TEST_WS" 2>/dev/null || return 1
      ...
      It "logs license check passed on success"
        When call verify.scan.license.run "$TEST_WS"
        The status should be success
