Describe "security/secret_scan.sh"
  ...
  Include "$BRIK_HOME/lib/stages/verify/scan/secret.sh"
  Include "$BRIK_HOME/spec/support/mock_helper.sh"

  Describe "verify.scan.secret.run"
    It "returns 6 for nonexistent workspace"
      When call verify.scan.secret.run "/nonexistent/workspace"
    ...
    Describe "Tier 1: command override"
      ...
      It "runs command override"
        When call verify.scan.secret.run "$TEST_WS"
    ...
    Describe "Tier 2: gitleaks"
      ...
      It "runs gitleaks detect"
        invoke_gitleaks() {
          verify.scan.secret.run "$TEST_WS" 2>/dev/null || return 1
