Describe "quality/lint.sh"
  ...
  Describe "verify.lint.run"
    It "returns 6 for nonexistent workspace"
      When call verify.lint.run "/nonexistent/workspace"
    ...
    Describe "unknown option"
      ...
      It "returns 2 for unknown option"
        When call verify.lint.run "$TEST_WS" --badopt
    ...
    Describe "with Node.js workspace and mock npx"
      ...
      It "passes --fix flag to eslint"
        invoke_fix() {
          verify.lint.run "$TEST_WS" --fix 2>/dev/null || return 1
          grep -q "\-\-fix" "$MOCK_LOG"
        }
