...
Describe "brik self-uninstall"
  ...
  Describe "--force with fake BRIK_HOME"
    setup_fake_home() {
      ...
      # Copy lib so brik can load its libs
      cp -R "${BRIK_HOME}/lib" "${FAKE_HOME}/lib" 2>/dev/null || true
  ...
  Describe "path safety - missing bin/brik in target"
    setup_no_bin() {
      ...
      cp -R "${BRIK_HOME}/lib" "${SCRIPT_HOME}/lib" 2>/dev/null || true
      ...
      cp -R "${BRIK_HOME}/lib" "${TARGET_HOME}/lib" 2>/dev/null || true
