Describe "build/node.sh - --tool alias for --package-manager"
  ...
  Describe "stacks.node.build with --tool"
    setup_tool_alias() {
      mock.setup
      TEST_WS="$(mktemp -d)"
    ...
    It "accepts --tool as alias for --package-manager"
      invoke_tool_alias() {
        stacks.node.build "$TEST_WS" --tool yarn 2>/dev/null || return 1
  ...
  Describe "stacks.node.install with --tool"
    setup_install_tool() {
      mock.setup
      TEST_WS="$(mktemp -d)"
    ...
    It "accepts --tool as alias for --package-manager in install"
      invoke_install_tool() {
        stacks.node.install "$TEST_WS" --tool pnpm 2>/dev/null || return 1
