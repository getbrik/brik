Describe "build/python.sh"
  ...
  Include "$BRIK_STACKS_LIB/python.sh"
  Include "$BRIK_HOME/spec/support/mock_helper.sh"

  Describe "_stacks.python._detect_pm"
    It "detects poetry from poetry.lock"
      When call _stacks.python._detect_pm "$WORKSPACES/python-poetry"
      The output should equal "poetry"
    End

    It "detects pipenv from Pipfile"
      When call _stacks.python._detect_pm "$WORKSPACES/python-pipenv"
      The output should equal "pipenv"
    End

    It "defaults to pip"
      When call _stacks.python._detect_pm "$WORKSPACES/python-simple"
    ...
    Describe "detects poetry from [tool.poetry] without poetry.lock"
      ...
      It "detects poetry from pyproject.toml tool.poetry section"
        When call _stacks.python._detect_pm "$TEST_WS"
        The output should equal "poetry"
      End
    End
  End

  Describe "stacks.python.build"
    It "returns 6 for nonexistent workspace"
      When call stacks.python.build "/nonexistent/workspace"
      The status should equal 6
      The stderr should include "required directory not found"
    End

    It "returns 2 for unknown option"
      When call stacks.python.build "$WORKSPACES/python-simple" --badopt foo
      The status should equal 2
      The stderr should include "unknown option"
    End

    It "returns 6 when no Python project file found"
      When call stacks.python.build "$WORKSPACES/unknown"
      The status should equal 6
      The stderr should include "no Python project file found"
    End

    It "returns 7 for unsupported tool"
      When call stacks.python.build "$WORKSPACES/python-simple" --tool conda
    ...
    Describe "require_tool pip failure"
      ...
      It "returns 3 when pip is not on PATH"
        When call stacks.python.build "$TEST_WS"
    ...
    Describe "with mock pip and pyproject.toml (python -m build available)"
      ...
      It "runs python -m build with pyproject.toml"
        invoke_pip_check() {
          stacks.python.build "$TEST_WS" 2>/dev/null || return 1
...
exit 0"
    ...
    Describe "explicit --tool override"
      ...
      It "forces pip on a poetry project when --tool pip specified"
        invoke_override() {
          stacks.python.build "$TEST_WS" --tool pip 2>/dev/null || return 1
          grep -q "python -m build" "$MOCK_LOG"
        }
...
Describe "test/python.sh"
  ...
  Describe "stacks.python.test_cmd"
    ...
    It "adds --junitxml when report_dir is provided"
      When call stacks.python.test_cmd "pytest" "/workspace" "/reports"
    ...
    It "ignores report_dir for unittest (no junit support)"
      When call stacks.python.test_cmd "unittest" "/workspace" "/reports"
      The output should equal "python -m unittest discover"
    End
    ...
    It "ignores report_dir for tox"
      When call stacks.python.test_cmd "tox" "/workspace" "/reports"
      The output should equal "tox"
    End

    It "returns 7 for unsupported framework"
      When call stacks.python.test_cmd "unknown" "/workspace" ""
      The status should equal 7
  ...
  Describe "stacks.python.test"
    It "delegates to pytest by default"
      When call stacks.python.test "/workspace" ""
      The output should equal "python -m pytest"
    End

    It "passes report_dir through"
      When call stacks.python.test "/workspace" "/reports"
      The output should equal "python -m pytest --junitxml=/reports/report.xml"
    End
  End
End
