Describe "condition.sh (portable condition evaluator)"
  ...
  # =========================================================================
  # conditions.eval - branch conditions (using BRIK_BRANCH)
  # =========================================================================
  Describe "conditions.eval"
    Describe "branch == exact match"
      ...
      It "matches when branch equals value"
        When call conditions.eval "branch == 'main'"
        The status should be success
      End

      It "does not match when branch differs"
        When call conditions.eval "branch == 'develop'"
    ...
    # =========================================================================
    # conditions.eval - tag conditions (using BRIK_TAG)
    # =========================================================================
    Describe "tag == exact match"
      ...
      It "matches exact tag"
        When call conditions.eval "tag == 'v1.0.0'"
        The status should be success
      End

      It "does not match different tag"
        When call conditions.eval "tag == 'v2.0.0'"
        The status should equal 1
      End
    End
    ...
    Describe "tag =~ glob match"
      Describe "with tag v2.3.1"
        ...
        It "matches v* glob"
          When call conditions.eval "tag =~ 'v*'"
          The status should be success
        End

        It "matches v2.* glob"
          When call conditions.eval "tag =~ 'v2.*'"
          The status should be success
        End
      End
      ...
      Describe "with tag release-1.0"
        ...
        It "matches release-* glob"
          When call conditions.eval "tag =~ 'release-*'"
          The status should be success
        End
      End
    ...
    # =========================================================================
    # conditions.eval - pipeline_source and custom vars
    # =========================================================================
    Describe "pipeline_source through full eval"
      ...
      It "matches pipeline_source condition"
        When call conditions.eval "pipeline_source == 'merge_request_event'"
        The status should be success
      End

      It "does not match different pipeline_source"
        When call conditions.eval "pipeline_source == 'push'"
        The status should equal 1
      End
    End
    ...
    Describe "custom environment variable through full eval"
      ...
      It "matches custom BRIK_ var condition"
        When call conditions.eval "BRIK_DEPLOY_TARGET == 'production'"
        The status should be success
      End

      It "does not match different value"
        When call conditions.eval "BRIK_DEPLOY_TARGET == 'staging'"
        The status should equal 1
      End
    End

    # =========================================================================
    # conditions.eval - special keywords
    # =========================================================================
    Describe "special keywords"
      It "returns 1 for 'manual'"
        When call conditions.eval "manual"
        The status should equal 1
      End
    End

    # =========================================================================
    # conditions.eval - whitespace handling
    # =========================================================================
    ...
    # =========================================================================
    # conditions.eval - AND/OR compound expressions
    # =========================================================================
    Describe "AND operator"
      ...
      Describe "when both sides are true"
        ...
        It "returns 0 when branch == 'main' AND tag == 'v1.0.0'"
          When call conditions.eval "branch == 'main' AND tag == 'v1.0.0'"
          The status should be success
        End
      End
      ...
      Describe "when right side is false"
        ...
        It "returns 1 when right condition is false"
          When call conditions.eval "branch == 'main' AND tag =~ 'v*'"
          The status should equal 1
        End
      End
    End
    ...
    Describe "OR operator"
      ...
      Describe "when left side is true"
        ...
        It "returns 0 when branch == 'main' OR branch == 'develop'"
          When call conditions.eval "branch == 'main' OR branch == 'develop'"
          The status should be success
        End
      End
    ...
    Describe "AND takes precedence over OR"
      ...
      Describe "when AND part matches (main with tag)"
        ...
        It "returns 0: (branch == 'main' AND tag =~ 'v*') OR branch == 'develop'"
          When call conditions.eval "branch == 'main' AND tag =~ 'v*' OR branch == 'develop'"
          The status should be success
        End
      End
  ...
  # =========================================================================
  # conditions.eval_deploy_env
  # =========================================================================
  Describe "conditions.eval_deploy_env"
...
YAML
      ...
      It "returns 0 when condition matches (production on main)"
        When call conditions.eval_deploy_env "production"
        The status should be success
      End

      It "returns 1 when condition does not match (staging needs tag)"
        When call conditions.eval_deploy_env "staging"
        The status should equal 1
      End
    End
...
YAML
    ...
    Describe "with nonexistent environment"
      ...
      It "returns 1 for unknown environment"
        When call conditions.eval_deploy_env "nonexistent"
        The status should equal 1
