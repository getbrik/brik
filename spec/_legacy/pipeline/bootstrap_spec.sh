Describe "bootstrap.sh"
  ...
  # ---------------------------------------------------------------------------
  Describe "_bootstrap._is_virtualized"
    Describe "returns true for gitlab"
      ...
      After 'cleanup_virt'
      It "returns 0"
        When call _bootstrap._is_virtualized
  ...
  # ---------------------------------------------------------------------------
  Describe "_bootstrap._detect_system_pkg_manager"
    Describe "when apt-get is available"
      ...
      It "returns apt-get (highest priority)"
        When call _bootstrap._detect_system_pkg_manager
  ...
  # ---------------------------------------------------------------------------
  Describe "_bootstrap._ensure_brik_bin"
    Describe "with valid BRIK_HOME"
      ...
      It "creates bin directory and adds to PATH"
        When call _bootstrap._ensure_brik_bin
        The status should be success
  ...
  # ---------------------------------------------------------------------------
  # _bootstrap._sys_pkg_install (CI dispatcher)
  # ---------------------------------------------------------------------------
  Describe "_bootstrap._sys_pkg_install"
    Describe "skips when tool already present"
      ...
      It "returns 0 without calling manager for bash"
        When call _bootstrap._sys_pkg_install "apk" "bash"
        The status should be success
    ...
    Describe "installs via apk when tool is missing"
      ...
      After 'mock.cleanup'
      It "calls apk for jq"
        When call _bootstrap._sys_pkg_install "apk" "jq"
        The status should be success
    ...
    Describe "rejects brew as unsupported"
      ...
      After 'mock.cleanup'
      It "returns 5"
        When call _bootstrap._sys_pkg_install "brew" "jq"
    ...
    Describe "dispatches to apt-get"
      ...
      After 'mock.cleanup'
      It "calls apt-get"
        When call _bootstrap._sys_pkg_install "apt-get" "git"
        The status should be success
    ...
    Describe "dispatches to yum"
      ...
      After 'mock.cleanup'
      It "calls yum"
        When call _bootstrap._sys_pkg_install "yum" "git"
        The status should be success
    ...
    Describe "dispatches to dnf"
      ...
      After 'mock.cleanup'
      It "calls dnf"
        When call _bootstrap._sys_pkg_install "dnf" "bash"
        The status should be success
