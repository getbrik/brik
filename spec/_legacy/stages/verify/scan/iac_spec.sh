Describe "security/iac.sh"
  ...
  Describe "verify.scan.iac.run"
    It "returns 6 for nonexistent workspace"
      When call verify.scan.iac.run "/nonexistent/workspace"
    ...
    Describe "Tier 1: command override"
      ...
      It "runs command override"
        When call verify.scan.iac.run "$TEST_WS"
    ...
    Describe "Tier 2: explicit tool (checkov)"
      ...
      It "runs checkov"
        invoke_tool() {
          verify.scan.iac.run "$TEST_WS" 2>/dev/null || return 1
